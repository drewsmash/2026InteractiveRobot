...
AC.VR.options = {
	...
	introSpins: 2,            // (number)   times the VR rotates 360 degrees in the intro
...
Object.extend(AC.VR.prototype, {
	...
	getEvent: function(event){
		if (event.touches) {
			...
			if (event.touches.length) {
				...
				// Fallback for touchend so throwing physics properly calculate velocity
	...
	unmakeInteractive: function(){
		...
		if (this.mobile) {
			var downEl = this.vr.down();
			if (downEl) {
				downEl.stopObserving('touchmove', this.bindGrabChange);
				downEl.stopObserving('touchend', this.bindGrabEnd);
			}
		}
	...
	recycle: function() {
		...
		if (this.grabHistoryInterval) clearInterval(this.grabHistoryInterval);
		delete this.frames;
...
AC.VR.Loader = Class.create({
	...
	onLoad: function (event) {
		if (!this.controller || !this.controller.vr || !this.controller.vr.frames) return;
